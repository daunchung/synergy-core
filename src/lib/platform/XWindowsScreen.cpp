...
void XWindowsScreen::setOptions(const OptionsList &options)
{
	for (UInt32 i = 0, n = options.size(); i < n; i += 2)
	{
		if (options[i] == kOptionXTestXineramaUnaware)
		{
			...
			LOG((CLOG_DEBUG "DAUN: XTest is Xinerama unaware %s", m_xtestIsXineramaUnaware ? "true" : "false"));
		}
...
void XWindowsScreen::warpCursor(SInt32 x, SInt32 y)
{
	...
	XEvent event;
	while (XCheckMaskEvent(m_display, PointerMotionMask | ButtonPressMask | ButtonReleaseMask | KeyPressMask | KeyReleaseMask | KeymapStateMask,
						   &event))
	{
		// do nothing
	}
...
void XWindowsScreen::setShape(SInt32 width, SInt32 height)
{
	...
	// follow xinerama, which is multi screen support for linux
	m_xinerama = false;
#if HAVE_X11_EXTENSIONS_XINERAMA_H
	int eventBase, errorBase;
...
void XWindowsScreen::handleSystemEvent(const Event &event, void *)
{
	...
	default:
		...
		if (m_xrandr)
		{
			if (xevent->type == m_xrandrEventBase + RRScreenChangeNotify || xevent->type == m_xrandrEventBase + RRNotify && reinterpret_cast<XRRNotifyEvent *>(xevent)->subtype == RRNotify_CrtcChange)
			{
...
void XWindowsScreen::processClipboardRequest(Window requestor,
...
XWindowsScreen::HotKeyItem::HotKeyItem(int keycode, unsigned int mask) : m_keycode(keycode),
																		 m_mask(mask)
{
	// do nothing
}
